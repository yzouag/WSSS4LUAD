...
for i in tqdm(visualize_pick):
    ...
    cam = np.argmax(cam_score, axis=0).astype(np.uint8)
    ...
    if not os.path.exists('ensemble_heatmap_nonorm'):
        os.mkdir('ensemble_heatmap_nonorm')
    plt.savefig(f'ensemble_heatmap_nonorm/{i:02d}.png')
    plt.close()
